...
def get_user(user_id, username):
    ...
    if row is None:
        cursor.execute("""INSERT INTO users (
            ...
            last_daily, last_weekly, vip_until, daily_streak,
            last_streak_claim, last_interest, created_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        (
...
def update_missions_played(user_id):
    cursor.execute(
        "UPDATE missions SET progress = progress + 1 WHERE user_id=? AND mission_name IN ('İlk Oyunun', '5 Oyun Oyna')",
        (user_id,)
    )
    conn.commit()
...
def update_missions_won(user_id):
    cursor.execute(
        "UPDATE missions SET progress = progress + 1 WHERE user_id=? AND mission_name='3 Oyun Kazan'",
        (user_id,)
    )
    conn.commit()
...
def get_missions(user_id):
    cursor.execute(
        "SELECT id, mission_name, progress, target, reward, claimed FROM missions WHERE user_id=? ORDER BY id ASC",
        (user_id,)
    )
    return cursor.fetchall()
...
def nav_main():
    rows = [
        [InlineKeyboardButton("🏠 Ana Menü", callback_data="back_main"), InlineKeyboardButton("🎮 Oyunlar", callback_data="menu_games")],
...
def main_menu():
    rows = [
        [InlineKeyboardButton("💰 Bakiye", callback_data="menu_balance"), InlineKeyboardButton("🎮 Oyunlar", callback_data="menu_games")],
...
def games_menu():
    rows = [
        [InlineKeyboardButton("🎡 Rulet", callback_data="info_rulet"), InlineKeyboardButton("🃏 Blackjack", callback_data="info_blackjack")],
...
async def safe_edit(message, text, reply_markup=None, min_interval=1.0):
    ...
    for _ in range(3):
        try:
            ...
            _last_edit_times[key] = time.monotonic()
            _last_edit_texts[key] = text
            return

        except RetryAfter as e:
            logging.warning(f"Rate limit yendi. {e.retry_after} sn bekleniyor.")
            await asyncio.sleep(e.retry_after + 0.5)

        except TimedOut:
            logging.warning("edit_text timeout verdi, tekrar denenecek.")
            ...
            logging.warning(f"safe_edit hata: {e}")
            await asyncio.sleep(1)

    try:
        await message.reply_text(text, parse_mode="HTML", reply_markup=reply_markup)
    except Exception as e:
        logging.warning(f"reply fallback da başarısız: {e}")

async def animated_panel(message, frames, delay=0.9, reply_markup=None):
    for i, frame in enumerate(frames):
        await safe_edit(
            message,
            frame,
            reply_markup if i == len(frames) - 1 else None,
            min_interval=0.9
        )
        if i != len(frames) - 1:
...
async def market(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for code, item in MARKET_ITEMS.items():
        text += (
            f"🎟 <b>{item['name']}</b>\n"
            f"💰 Fiyat: <b>{format_number(item['price'])} 🪙</b>\n"
...
async def rulet(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await safe_edit(msg, text, nav_main(), min_interval=1.0)
...
async def crash(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    visible_steps = [1.10, 1.25, 1.50, 1.80, 2.10, 2.50, 3.00, 3.50]
    ...
    if multiplier >= 2.0:
        ...
        text = (
            f"🚀 <b>CRASH SONUCU</b>\n\n"
            f"Çarpan: <b>x{multiplier:.2f}</b>\n"
            f"🎉 +{format_number(profit)} 🪙{lvl}"
        )
    else:
