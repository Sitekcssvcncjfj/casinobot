...
async def animated_panel(message, frames, delay=0.6, reply_markup=None):
    for i, frame in enumerate(frames):
        ...
        if i != len(frames) - 1: await asyncio.sleep(delay)
...
async def highlow(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await animated_panel(msg, ["📈 <b>Rastgele sayı üretiliyor...</b>", "📉 <b>Grafik hareket ediyor...</b>", "📊 <b>Sonuç açılıyor...</b>"], 0.6)
    if number == 50:
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_profile":
        ...
        return
    if data == "menu_games":
        await safe_edit(query.message, "╔══════════════════════╗\n      🎮 <b>OYUN SALONU</b>\n╚══════════════════════╝\n\nŞansını denemek istediğin oyunu seç.\nAnimasyonlu oyun bilgileri aşağıda.", games_menu())
        return
    if data == "menu_bank":
        last_interest = get_last_interest(user.id)
        remain = interest_remaining(last_interest)
